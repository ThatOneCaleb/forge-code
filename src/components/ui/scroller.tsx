...
import { useComposedRefs } from "./scroller-utils/compose-refs";
...
function Scroller(props: ScrollerProps) {
  ...
  const composedRef = useComposedRefs(ref as React.Ref<HTMLDivElement>, containerRef);
        ...
        if (withNavigation) {
          setScrollVisibility((prev) => {
            ...
            if (prev.up !== newUp || prev.down !== newDown) {
              return { ...prev, up: newUp, down: newDown };
            }
            return prev;
          });
        }
